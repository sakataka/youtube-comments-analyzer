...
export type EvidenceComment = { local?: { sentiment: string; polarity: number; emotion: string | null; people: Record<string, string> } | null; person_judgements?: Record<string, PersonJudgement>; comment_id: string; text_original: string; like_count: number; published_at: string; is_reply: boolean; parent_text: string | null; review_reason: string | null; analysis_status: string; observations: Observation[]; subtitles: Array<{ id: string; start: number; end: number; text: string }>; url: string };
...
export type LightReport = Omit<OpinionReport, 'schema_version' | 'analysis' | 'groups' | 'targets' | 'summary' | 'method'> & {
  schema_version: 'report.v4'; summary_status: string; jev?: JevReport; person_statistics?: PersonStatistics; insights?: Insights; local?: LocalReport; x_pulse?: XPulse;

...
プロジェクト直下の `.env` に `TYPESAFE_API_KEY=取得したキー` を追加します。キーはリクエスト時に読み込むため、追加後のサーバー再起動は不要です。環境変数が設定されている場合はそちらを優先します。画面を再読み込みし、取得済みレポートの「Jevで分類する（最大100件）」で実行します。

本文・返信先・動画タイトルをTypeSafeへ送り、質問／要望／感想／情報などの種類と、コメント全体の論調を分類します。投稿者名・IDは送信しません。人物別評価や既存の要約・集計とは別の任意機能です。

コメントIDのハッシュ順で最大100件を固定抽出します。全体を代表する標本ではありません。本文2,000文字、返信先1,000文字が上限で、省略は画面に表示します。種類の確信度0.7未満は保留、評価は分類候補を「暫定」として表示します。この閾値は日本語の正答率を保証しません。

1回最大100リクエスト・5分、通信は1件最大20秒で、自動再試行はしません。同じ入力・モデル・判定条件の結果はrun内で再利用します。途中停止・失敗時も完了分は保存し、再実行で再利用できます。累計リクエスト試行数とAPIが返したトークン数を表示します。失敗した通信の使用量は取得できない場合があります。料金はTypeSafe側の契約で確認してください。キーはブラウザやGitに含めません。

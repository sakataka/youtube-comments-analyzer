# 実装ロードマップ

この文書は [requirements.md](requirements.md) を基準に、まだ実装・検証・改善が残っている項目だけを残した backlog です。

完了済みの実装詳細はここには載せません。現状の使い方と実装済み機能は [../README.md](../README.md) を参照してください。

## API / データモデルの残り

現時点の MVP backlog は空です。次の実データ検証で見つかった分析品質・運用上の課題をここに追加します。

## GitHub 登録前チェック

- README のセットアップ、起動、最小動作確認コマンドを現行実装に合わせる。
- `.env.example` に公開 repo で必要な環境変数の枠を揃える。
- ローカル運用メモを `docs/` 配下の公開向け troubleshooting として整理する。
- ライセンスを明記する。
- `bun run test` と `bun run build` が通ることを確認する。

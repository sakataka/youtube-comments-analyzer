...
公開コメントを全件集計し、最大250件の抽出コメントをGPT-6 Sol（`gpt-6-sol` / `medium`）で要約する個人用ローカルアプリです。全件の検索・並べ替え・集計はAIなしで利用できます。AIは要約と人物辞書に通常各1回、双方の修復込み最大4回です。
...
## 上位と全体・場面・目立つコメント（AIなし）

取得全件から、次の項目をAIを使わずにレポートを開くたびに計算します。

- **上に見えるコメントと、全体のずれ**：いいね上位20件の親コメントが全いいねに占める割合と、いいねが0の親コメントの件数を表示します。人物集計が完了していれば、人物ごとに「全件での言及率」「上位20件中の件数」「いいねの占有率」を並べます。YouTubeの人気順はいいね数だけで決まらないため、上位20件は近似です。
- **目立つコメント**：いいね上位、返信の多い親コメント、上位20件の外で1時間あたりのいいねが多い投稿、いいねの多い返信を各5件まで表示します。Jevの5段階分類があれば、否定・賛否混在と分類されたのにいいねを集めた投稿も表示します。
- **場面**：本文中の「12:34」「1:02:03」を1〜3件含むコメントを時刻の区間ごとに数えます。時刻を4件以上並べた目次のような投稿は数えません。棒か「この場面の原文を読む」を押すと、その区間に触れた原文をいいね順で表示します（comments APIの`moment_start`・`moment_end`）。新しく取得した動画では動画の長さを超える時刻を除外します。
- **投稿時間**：動画の公開から投稿までの経過時間ごとの件数です。いいねは取得時点の累計なので、早い投稿ほど多くなりやすい点に注意してください。

## 全件の賛否・感情とXの反応（自動）

分析が終わると、続けて2つの処理を自動で実行します。

- **ローカル判定**：このMacで日本語の分類モデルを動かし、取得全件の賛否（肯定・中立・否定）と、いちばん強い感情（喜び・期待・驚きなど8種）を判定します。人物への評価は、名前を含む文ごとに判定します。AIのトークンは使いません。
- **Xの反応**：ログイン済みのGrok CLIでXを1回検索し、この動画への反応と、YouTubeコメントとの違いを要約します。

どちらも参考値で、失敗しても要約・集計・原文はそのまま使えます。初回はモデルのダウンロード（約1GB）が必要です。モデルの選び方、判定のしきい値、設定（`LOCAL_MODELS` / `X_SEARCH`）は[ローカル判定とX検索](docs/local-models-and-x.md)を参照してください。
...
プロジェクト直下の `.env` に `TYPESAFE_API_KEY=取得したキー` を追加します。キーはリクエスト時に読み込むため、追加後のサーバー再起動は不要です。環境変数が設定されている場合はそちらを優先します。画面を再読み込みし、取得済みレポートの「Jevで分類する（最大500件）」で実行します。

本文・返信先・動画タイトルをTypeSafeへ送り、質問／要望／感想／情報などの種類と、コメント全体の論調を分類します。投稿者名・IDは送信しません。人物別評価や既存の要約・集計とは別の任意機能です。

取得済みコメントのいいね数が多い順に最大500件を選びます。同数の場合はコメントID順です。全体を代表する標本ではありません。本文2,000文字、返信先1,000文字が上限で、省略は画面に表示します。種類の確信度0.7未満は保留、評価は分類候補を「暫定」として表示します。この閾値は日本語の正答率を保証しません。

最大10並列、1回最大500リクエスト・5分、通信は1件最大20秒で、自動再試行はしません。同じ入力・モデル・判定条件の結果はrun内で再利用します。途中停止・失敗時は追加送信を止め、送信済みの処理を待って完了分を保存し、再実行で再利用できます。累計リクエスト試行数とAPIが返したトークン数を表示します。失敗した通信の使用量は取得できない場合があります。料金はTypeSafe側の契約で確認してください。キーはブラウザやGitに含めません。

### 5段階の評価分類

Jev分類器は「とてもポジティブ／ややポジティブ／ニュートラル／ややネガティブ／とてもネガティブ」に加え、「賛否が混在」「判定困難」を別分類にします。棒グラフの割合の分母は分類済み件数のみです。グラフを押すと原文を絞り込め、種類・本文検索も併用できます。原文は20件ずつ表示します。

異なる対象への賛否を中立へ平均せず、文脈不足・皮肉の不明さは判定困難へ分けます。評価の強さと確信度は別です。個々の人物への評価を示すものではありません。旧分類は旧分類として表示し、新条件での再実行時にキャッシュを区別します。

...
hover / focusは短い色変化とリングで示す。body等は120ms ease-out、共通ボタンは100ms。CSSのreduced-motionではアニメーションと遷移を0.01msへ短縮し、htmlのsmooth scrollをautoへ変更する。v4レポートのセクション移動はこの設定に応じて即時スクロールへ切り替え、移動先の見出しへフォーカスする。
...
## 承認済みの操作改善（2026-09-21）

- v4レポートに全件集計・Jev分類・人物集計・話題と要約・原文の固定追従ナビを設置。狭い幅では折り返し、移動先見出しに余白を取る。
- 話題から原文へ移動すると選択した話題名を表示し、原文件数の更新を通知する。
- Jevは該当件数と実際に表示した件数を分けて表示する。

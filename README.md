...
現在の実装は MVP-2 の一部です。YouTube Data API または fixture からコメントを保存し、SudachiPy ベースの日本語解析と accepted alias による deterministic な集計を行います。新規実行時はコメント取得、自動割り当て、Codex app server 経由の AI 確認、候補・alias の自動補正まで先に進め、曖昧コメントは人間が要確認で見てから最終分析へ進みます。
...
- 新規分析時に LLM 補助分析と候補・alias 提案の自動補正まで先に実行する
...
6. 「コメント取得と自動チェックを開始」を押す。
7. Data Source が `Cache` の場合は同条件の保存済みデータを使っており、YouTube API は再消費していない。
8. 自動割り当て、AI 確認、自動補正が終わるまで待つ。AI 確認だけ失敗した場合も、自動割り当ての人間チェックから続けられる。
9. `要確認` と `候補確認` で、人間確認が必要なものだけ採用・除外、表示名編集、表記追加、統合、集計からの除外を行う。
10. `人間チェックを完了した` にチェックを入れ、`人間チェックを終えて分析` を押す。
11. タブを切り替えて、概要ダッシュボード、言及ランキング、取得範囲、返信件数、いいね数分布、未知 alias 候補、人物別詳細、代表コメント、section status を確認する。
12. 必要に応じて `AIインサイト` タブで、集計済みサマリーからコメント状況の発見を抽出する。
13. `頻出語レビュー` タブで、人物の別表記なら紐づけ先を選んで `alias に追加` する。
14. 必要に応じて `LLM補助` タブで `LLM 補助を再実行` を押し、候補整理、alias 補完案、曖昧コメント分類を確認する。
15. `コメント` タブで本文検索や人物フィルタを使い、根拠コメントを確認する。
16. 必要に応じてコメント単位で人物紐づけを追加または解除する。
17. 以前の結果を確認したい場合は、折りたたまれている `過去分析` を開いて run を選ぶ。
...
- `AIインサイト`: 分析確定後に、集計済みサマリーからコメント状況の発見、注意点、深掘り質問を AI で抽出できます。
- `人物詳細`: 人物ごとの魅力カテゴリ、tone、評価語文脈、要約、根拠コメント、TF-IDF 的に重み付けした特徴語を確認できます。
...
- `運用・設定・データ管理`: `YOUTUBE_API_KEY` の読み込み有無、LLM / embedding 状態、保存データ容量、現在 run の JSON export、run/cache の退避・削除を確認できます。

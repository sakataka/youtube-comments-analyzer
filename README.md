...
4. 返信コメントを含めたい場合は `API同梱分を含める` または `返信を全件取得` を選ぶ。
...
10. 「候補を確定して集計」を押す。
11. 概要ダッシュボード、言及ランキング、取得範囲、返信件数、いいね数分布、未知 alias 候補、人物別詳細、代表コメント、section status を確認する。
12. 未知 alias 候補が人物の別表記なら、紐づけ先を選んで `alias に追加` する。
13. 必要に応じて `LLM 補助を実行` を押し、候補整理、alias 補完案、曖昧コメント分類を確認する。
14. コメント一覧で本文検索や人物フィルタを使い、根拠コメントを確認する。
15. 必要に応じてコメント単位で人物紐づけを追加または解除する。
16. 以前の結果を確認したい場合は、`過去分析` から run を開く。
...
`reply_fetch_mode=inline_subset` は `commentThreads.list` のレスポンスに同梱される返信だけを保存します。全返信を保証するものではありませんが、追加の `comments.list` 呼び出しなしで返信欄の一部を分析対象にできます。`reply_fetch_mode=full` は `comments.list` で返信をページング取得するため、返信数に応じて YouTube API quota を追加消費します。

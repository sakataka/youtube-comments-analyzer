...
現在の実装は MVP-2 の一部です。YouTube Data API または fixture からコメントを保存し、SudachiPy ベースの日本語解析と accepted alias による deterministic な集計を行います。新規実行時はコメント取得、自動割り当て、Codex app server 経由の AI 確認、自動補正まで先に進め、人間は要確認だけを見てから最終分析へ進みます。
...
- 新規分析時に LLM 補助分析と反映可能な提案の自動補正まで先に実行する

...
export function LightReportView({ report, error, working, action, onNewAnalysis, onOpenSettings, onOpenRun }: {
  ...
  }, [report.run_id, report.coverage.fetched, group, person, stance, moment, search, sort, offset, report.coverage.updated_at]);
  ...
  function filter(id: string) {
    setPerson(''); setStance(''); setMoment(''); setGroup(id); setOffset(0); setSearch('');
  ...
  return <main className="opinion-shell">
    ...
    <nav className="report-section-nav" aria-label="分析結果のセクション">{[['report-counts', '全件集計'], ['report-visible', '上位と全体'], ['report-moments', '場面・時間'], ['report-jev', 'Jev分類'], ['report-people', '人物集計'], ['report-topics', '話題・要約'], ['raw-comments', '原文']].map(([id, label]) => <a key={id} href={`#${id}`} onClick={event => { event.preventDefault(); goToSection(id); }}>{label}</a>)}</nav>
    ...
    <VisibleVsAllView insights={report.insights} onPerson={id => { setGroup(''); setMoment(''); setPerson(id); setStance(''); setSearch(''); setOffset(0); goToSection('raw-comments'); }} />
    <MomentsView insights={report.insights} videoUrl={report.video.url} selected={moment} onMoment={(start, end) => { setGroup(''); setPerson(''); setStance(''); setSearch(''); setOffset(0); setMoment(`${start}-${end}`); setSort('likes'); goToSection('raw-comments'); }} />
    <JevClassification key={report.run_id} result={report.jev} running={running} working={working} action={action} />
    <PersonStatisticsView statistics={report.person_statistics} running={running} working={working} action={action} onFilter={(id,label) => {setGroup('');setMoment('');setPerson(id);setStance(label);setSearch('');setOffset(0);goToSection('raw-comments');}} />
    ...
    <section id="raw-comments" className="opinion-section"><div className="opinion-section-heading"><span>04 / ORIGINAL COMMENTS</span><h2 tabIndex={-1}>原文を読む</h2></div>
      ...
      {person ? <p>絞り込み：{report.person_statistics?.people?.find(p => p.id === person)?.name} {stance ? `／ ${stanceLabels[stance]}` : ''}</p> : null}
